 ...
 * is cut, the thermistor isolated from the circuit and its resistance is measured 
 * at several points in the temperature scale. Inside the smoker a calibrated 
 * Taylor oven thermometer is hanging from a rack near the sensor to be measured.
 * This process is repeated at other set points in order to create a representative
 * curve of (R,T) values. 
 * 
 ...
 * a task is to be active. If not it simply returns. This works because the tasks
 * are ordered by increasing time interval.
 * The tasks:
...
#define PROGRAM_NAME "ThermoSetter V5"
...
void setup()
{
	...
	for (int i=0; i>3; i++) {   // discard a few reads as per spec.
	...
	pinMode(MAIN_SWITCH_PIN, INPUT_PULLUP);  // switch to GND: HIGH = off, LOW = on
	...
	setStoveOff();		// to be safe
}

/**********************************************************
 * loop()
 * Several tasks are performed at increasing time intervals.
...
//
	mainsw = digitalRead(MAIN_SWITCH_PIN);
	#if (DEBUG_LVL >= 4)
	Serial.write(strbuf, snprintf(strbuf, sizeof(strbuf),
		"==> mainsw: %d\n", mainsw ));
	#endif	
	if (displayActive) { 
		if (mainsw == HIGH) {		// pullup w/ open circuit to GND
			quiesceDisplay();	// stop display
		}
	} else { 
		if (mainsw == LOW) {		// pullup w/ closed circuit to GND
			resetDisplay();	 	// restart display
		}
	}
...
*/
	if ((setpot.avg_temperature < standby_threshold) || (displayActive == false) ) {
		#if (DEBUG_LVL >= 2)
...
/*
 ...
 Calculate the slope of dT/dt, where dT is delta temperature and dt is delta time.
 The time interval is the time since the last checkpoint. No need to integrate.
 This slope is a rough guess of where the temperature will be in the next unit interval.
 Since dt is constant (say unit 1 minute) it can be ignored, using just delta temp
 as the magnitude of the slope.
...
void displayStatus() {
	...
	#if (DEBUG_LVL >= 1)
	Serial.write(strbuf, snprintf(strbuf, sizeof(strbuf),
	  "HH:MM %s  O:%d  %s  M:%d  %s Set:%d  %s HE:%c SB:%c SW:%d\n", 
	  hhmm_str, stove.avg_reading, stove.display_str, 
	  meat.avg_reading, meat.display_str, setpot.avg_reading, 
	  setpot.display_str, heat_indicator, standby_indicator,mainsw) );
	#endif
	...
	lcd.setCursor(0, 0);
	lcd.print(hhmm_str);
	#ifdef DISPLAY_ADC_READING	// show the ADC values (debug)
	lcd.setCursor(8, 0);
	...
	#else	                	// show the calculated temperature
	lcd.setCursor(8, 0);
